	...
	throw ( Error )
{
	...
	try 
	{
		...
		if ( pResult->next() )
		{
			...
			"SELECT WorldID, Host, DB, User, Password FROM WorldDBInfo");
			cout << "[LOGIN_SERVER] query WorldDBInfo" << endl;
		#else
			pResult = pStmt->executeQuery( 
			"SELECT WorldID, Host, DB, User, Password FROM WorldDBInfo WHERE WorldID = 0");
		#endif
	...
	throw ( DuplicatedException )
{
	...
	map< int , Connection * >::iterator itr = m_DistConnections.find( TID );
	...
	throw ( DuplicatedException )
{
	...
	map< int , Connection * >::iterator itr = m_CBillingConnections.find( TID );
	...
	throw ( NoSuchElementException )
{
	...
	map<int, Connection*>::iterator itr = m_DistConnections.find(Thread::self());
