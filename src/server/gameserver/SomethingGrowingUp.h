...
public:
	...
	ExpTable() { m_Records.reserve(MaxLevel); }
	virtual ~ExpTable() { }
...
public:
	...
	bool increaseExp( GoalExpType exp, bool canLevelUp = true );
	bool levelUp();
	bool levelDown();
...
template <class ExpTableClass>
bool SomethingGrowingUp<ExpTableClass>::increaseExp( GoalExpType exp, bool canLevelUp )
{
	if ( m_GoalExp <= exp )
	{
