...
public:
	...
	Effect* findEffect(Effect::EffectClass EClass) throw();
	Effect* findEffect(ObjectID_t ObjectID) throw();

	// Enemy Erase 이펙트를 가려내기 위한 함수이다.
	Effect* findEffect(Effect::EffectClass EClass, string EnemyName) throw();
