...
public:
	...
	virtual void affect(Zone* pZone, ZoneCoord_t x, ZoneCoord_t y, Object* pTarget) throw(Error) { throw UnsupportedError(); }
	virtual void affect(Item* pItem) throw(Error) { throw UnsupportedError();}
	
	virtual void unaffect(Item* pItem) throw(Error) { throw UnsupportedError(); }
	virtual void unaffect() throw(Error) = 0;
	virtual void unaffect(Creature* pCreature) throw(Error) { throw UnsupportedError(); } // 원래는 pure virtual이었는데 EffectRelicTable때메 바꿨다. by sigi
	virtual void unaffect(Zone* pZone, ZoneCoord_t x, ZoneCoord_t y, Object* pTarget) throw(Error) { throw UnsupportedError(); }
