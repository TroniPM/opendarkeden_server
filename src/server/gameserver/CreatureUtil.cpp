	...
	throw()
{
	...
	else if (pCreature->isMonster())
	{
		...
		if ( pOusters != NULL )
		{
			return getPercentValue((int)( pMonster->getOustersExp( pOusters ) ), g_pVariableManager->getVariable( MONSTER_EXP_RATIO ) );
		}
	...
	BEGIN_DB
	{
		...
		Result* pResult = pStmt->executeQuery(
				"SELECT GuildID FROM %s where Name='%s'", 
					(race==RACE_SLAYER? "Slayer" : "Vampire"), Name.c_str());

		if (pResult->next()) 
		{
			guildID = (GuildID_t)pResult->getInt(1);

			if (guildID==0 || guildID==99)
			{
				SAFE_DELETE(pStmt);
				return false;
			}
...
bool canSee( Creature* pSource, Creature* pTarget, EffectObservingEye* pEffectObservingEye, EffectGnomesWhisper* pEffectGnomesWhisper )
{
...
void addSimpleCreatureEffect( Creature* pCreature, Effect::EffectClass eClass, int time, bool isSend)
{
	...
	if ( isSend )
	{
		...
		else gcAddEffect.setDuration( time );
