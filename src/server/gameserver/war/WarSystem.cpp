	...
	throw (Error)
{
	...
	if (pWar->getWarType()==WAR_GUILD)
	{
		...
		m_ActiveWars.push_back( ActiveWarInfo(pGuildWar->getCastleZoneID(), pGuildWar->getChallangerGuildID() ) );
	...
	if (pWork!=NULL)
	{
		...
		if (pWar->getWarType()==WAR_GUILD)
		{
			...
			list<ActiveWarInfo>::iterator itr = find( m_ActiveWars.begin(), 
													m_ActiveWars.end(), 
													ActiveWarInfo( pGuildWar->getCastleZoneID() ) );
	...
	throw(Error)
{
	...
	for( ; itr != schedules.end(); itr++ )
	{
		...
		if (pWar->getWarType()==WAR_GUILD)
		{
			...
			if( pGuildWar->getCastleZoneID() == zoneID )
			{
				return pWarSchedule;
			}
		}
