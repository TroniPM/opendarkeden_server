...
void EffectDestructionSpear::affect(Creature* pCreature) throw(Error)
{
	...
	&& !(pZone->getZoneLevel() & COMPLETE_SAFE_ZONE) )
	{
		if ( pCreature->isPC() )
		{
			...
			::setDamage( pPC, getDamage(), pCastCreature, SKILL_DESTRUCTION_SPEAR, &gcMI, &gcAttackerMI );
			pPC->getPlayer()->sendPacket( &gcMI );
		}
		else if ( pCreature->isMonster() )
		{
			::setDamage( pCreature, getDamage(), pCastCreature, SKILL_DESTRUCTION_SPEAR, NULL, &gcAttackerMI );
		}
