	...
	try 
	{
		...
		if (bBulletCheck) 
		{
				...
				if (bHitRoll && bTimeCheck && bRangeCheck && bPK) 
				{
					...
					if (!pTargetCreature->isSlayer())
					{
						if ( bIncreaseDomainExp )
						{
							...
							increaseDomainExp(pSlayer, DomainType , 1, _GCAttackArmsOK1);
						}
			...
			else
			{
				...
				decreaseDurability(pSlayer, NULL, 1, &_GCSkillToTileOK1, NULL);
	...
	throw(Error)
{
	...
	try 
	{
		...
		if (HitRoll::isSuccess(pMonster, pEnemy) && verifyDistance(pMonster, pEnemy, pMonster->getMissileRange())) 
		{
			...
			// 공격자와 상대의 아이템 내구성 떨어트림.
			decreaseDurability(pMonster, pEnemy, 1, NULL, &_GCAttackArmsOK2);
