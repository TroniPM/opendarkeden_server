...
// 직접적으로 데미지를 세팅한다.
HP_t setDamage(Creature* pTargetCreature, Damage_t Damage, Creature* pAttacker, SkillType_t SkillType = 0, ModifyInfo* pMI = NULL, ModifyInfo* pAttackerMI = NULL, bool canKillTarget = true );

// 아이템 내구도를 떨어뜨린다.
void decreaseDurability(Creature* pCreature, Creature* pTargetCreature, WORD point, ModifyInfo*, ModifyInfo*);
...
// 슬레이어 계열 경험치를 계산한다.
bool increaseDomainExp(Slayer* pSlayer, SkillDomainType_t Domain, Exp_t Exp, ModifyInfo &);
