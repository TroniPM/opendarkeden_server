	...
	throw(Error)
{
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bPK)
		{
			...
			decreaseDurability(pSlayer, pTargetCreature, 1, &_GCSkillToObjectOK1, &_GCSkillToObjectOK2);
...
//			Damage += computeMagicDamage(pTargetCreature, param.SkillDamage, param.SkillType, true);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bCanHit && bPK && bSatisfyRequire )
		{
			...
			if (bCanSeeCaster) 
			{
				...
				decreaseDurability(pOusters, pTargetCreature, pSkillInfo->getPoint(), &_GCSkillToObjectOK1, &_GCSkillToObjectOK2);
			}
			else
			{
				...
				decreaseDurability(pOusters, pTargetCreature, pSkillInfo->getPoint(), &_GCSkillToObjectOK1, &_GCSkillToObjectOK6);
			}
	...
	try
	{
		...
		if (bRangeCheck && bHitRoll && bCanHit) 
		{
			...
			if (bCanSeeCaster)
			{
				...
				decreaseDurability(pMonster, pEnemy, pSkillInfo->getPoint(), NULL, &_GCSkillToObjectOK2);
			}
			else
			{
				setDamage(pEnemy, Damage, pMonster, param.SkillType, &_GCSkillToObjectOK6);
				decreaseDurability(pMonster, pEnemy, pSkillInfo->getPoint(), NULL, &_GCSkillToObjectOK6);
			}
