	...
	CEffectID_t CEffectID, bool bForceKnockback) throw (Error)
{
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck )
		{
				...
				if (rect.ptInRect(tileX, tileY))
				{
					...
					for(; itr != targetList.end(); itr++)
					{
						...
						if (pTargetCreature != pSlayer)
						{
							...
							if (pTargetCreature->isFlag(Effect::EFFECT_CLASS_NO_DAMAGE) // by sigi. 2002.10.30
			...
			// 공격자의 아이템 내구성을 떨어뜨린다.
			decreaseDurability(pSlayer, NULL, pSkillInfo->getPoint(), &_GCSkillToTileOK1, NULL);
			...
			for (list<Creature*>::const_iterator itr = cList.begin(); itr != cList.end(); itr++)
			{
				...
				if (pTargetCreature->isPC())
				{
					...
					// 아이템의 내구력을 떨어뜨린다.
					decreaseDurability(NULL, pTargetCreature, pSkillInfo->getPoint(), NULL, &_GCSkillToTileOK2);
	...
	CEffectID_t CEffectID, bool bForceKnockback, int HitBonus) throw (Error)
{
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck )
		{
			...
			// 공격자의 아이템 내구성을 떨어뜨린다.
			decreaseDurability(pVampire, NULL, pSkillInfo->getPoint(), &_GCSkillToTileOK1, NULL);
				...
				if (rect.ptInRect(tileX, tileY))
				{
					...
					for(; itr != targetList.end(); itr++)
					{
						...
						if (pTargetCreature != pOusters)
						{
							...
							bool bRaceCheck      = pTargetCreature->isVampire() || pTargetCreature->isSlayer() || pTargetCreature->isMonster();
							bool bZoneLevelCheck = checkZoneLevelToHitTarget(pTargetCreature);
...
//									Damage += computeMagicDamage(pTargetCreature, param.SkillDamage, param.SkillType, true);
			...
			// 공격자의 아이템 내구성을 떨어뜨린다.
			decreaseDurability(pOusters, NULL, pSkillInfo->getPoint(), &_GCSkillToTileOK1, NULL);
		...
		if (bRangeCheck && bHitRoll)
		{
			...
			for (list<Creature*>::const_iterator itr = cList.begin(); itr != cList.end(); itr++)
			{
				...
				if (pEnemy->isPC())
				{
					...
					// 아이템의 내구력을 떨어뜨린다.
					decreaseDurability(NULL, pEnemy, pSkillInfo->getPoint(), NULL, &_GCSkillToTileOK2);
