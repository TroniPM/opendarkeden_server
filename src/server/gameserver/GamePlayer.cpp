     ...
     throw (IOException , Error)
{
	...
	try {
		...
		while (true) 
		{
			...
			{
				filelog("GamePlayer.txt", "Packet ID exceed MAX, RECV [%d/%d],Name[%s],Host[%s]", 
					packetID, 
					Packet::PACKET_MAX,
			...
			try 
			{
				...
				{
					filelog("GamePlayer.txt", "Not Valid Packet, RECV [%d],Name[%s],Host[%s]", 
						packetID, 
						getCreature()->getName().c_str(), 
						getSocket()->getHost().c_str());
				...
				{
					filelog("GamePlayer.txt", "Too Larget Packet Size, RECV [%d],PacketSize[%d],Name[%s],Host[%s]", 
						packetID, packetSize,
						getCreature()->getName().c_str(), 
						getSocket()->getHost().c_str());
