...
// 능력치 제한 체크는 해야하는 경우
	...
	try 
	{
		...
		*/
		...
		{
			pResult = pStmt->executeQuery(
				"SELECT ZoneID, Slot, SwordLevel+BladeLevel+GunLevel+EnchantLevel+HealLevel FROM Slayer WHERE Name = '%s' AND PlayerID = '%s' AND Active = 'ACTIVE'", 
				pPacket->getPCName().c_str() ,
				pLoginPlayer->getID().c_str()
			);
		}
		...
		{
			pResult = pStmt->executeQuery(
				"SELECT ZoneID, Slot, Level FROM Vampire WHERE Name = '%s' AND PlayerID = '%s' AND Active = 'ACTIVE'", 
				pPacket->getPCName().c_str() ,
				pLoginPlayer->getID().c_str()
			);
		}
		...
		{
			pResult = pStmt->executeQuery(
				"SELECT ZoneID, Slot, Level FROM Ousters WHERE Name = '%s' AND PlayerID = '%s' AND Active = 'ACTIVE'", 
				pPacket->getPCName().c_str() ,
				pLoginPlayer->getID().c_str()
			);
		}
...
//			g_pGameServerManager->sendPacket(pGameServerInfo->getIP() , 3335 , &lgIncomingConnection);
		...
		pStmt1->executeQuery(	// (!)
		//pStmt->executeQuery(
			"UPDATE Player Set LastSlot=%d WHERE PlayerID = '%s'",
			slot,
			pLoginPlayer->getID().c_str()
		);
