...
//			cout << "이상한 글자" << endl;
		...
		if (bFreePass)	// by sigi. 2002.10.23
		{
...
addLoginPlayerData(const string& ID, const string& ip, const string& SSN, const string& zipcode)
{
	...
	BEGIN_DB
	{
		...
		*/
		sql << "INSERT INTO LoginPlayerData (PlayerID,IP,SSN,ZipCode,Date,Time) VALUES ('"
			<< ID << "','" << ip << "','" << SSN << "','" << zipcode << "','"
			<< currentDT.substr( 0, 10 ).c_str() << "','" << currentDT.substr( 11 ).c_str() << "')";
...
//		int classD    = atoi(ip.substr(k+1, ip.size()-k-1).c_str());
		...
		Result* pResult = pStmt->executeQuery(
			"SELECT class, first, last FROM IPBlockInfo WHERE (IP = '%s' AND class=1) OR (IP = '%s' AND class=2) OR (IP = '%s')",
