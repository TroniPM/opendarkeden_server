	 ...
	 throw (ProtocolException , Error)
{
	...
	try
	{
		...
		pResult = pStmt->executeQuery("SELECT PlayerID, CurrentServerGroupID, LogOn, SpecialEventCount, PayType, PayPlayDate, PayPlayHours, PayPlayFlag, BillingUserKey, FamilyPayPlayDate FROM Player WHERE PlayerID = '%s'",
										pGamePlayer->getID().c_str());
...
// by sigi. 2002.11.18. 제한적 무료 사용자. - -; 일단 login
	...
	*/
	...
	bool isMasterLair = pZoneInfo->isMasterLair();
	...
	if ( pCreature->isPC() && pCreature->isPLAYER() )
	{
		...
		if ( isCastleZone || isMasterLair || isPKZone || isMaze || isEventZone )
		{
