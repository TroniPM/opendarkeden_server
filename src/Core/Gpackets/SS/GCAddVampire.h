...
{
public:
	GCAddVampire() throw() : m_pEffectInfo(NULL), m_pPetInfo(NULL) { m_FromFlag = 0; }
	GCAddVampire(const PCVampireInfo3 & vampireInfo) throw() : m_VampireInfo(vampireInfo), m_pEffectInfo(NULL), m_pPetInfo(NULL) { m_FromFlag = 0; }
...
public:
	...
	PacketSize_t getPacketSize() const throw() { return m_VampireInfo.getSize() + m_pEffectInfo->getSize() + ((m_pPetInfo!=NULL)?m_pPetInfo->getSize():szPetType) + szBYTE; }
...
public:
	...
	PacketSize_t getPacketMaxSize() const throw() { return PCVampireInfo3::getMaxSize() + EffectInfo::getMaxSize() + PetInfo::getMaxSize() + szBYTE; }
};
